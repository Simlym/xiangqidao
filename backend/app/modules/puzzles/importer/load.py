...
运行（在 backend/ 目录下）:
    python -m app.modules.puzzles.importer.load seeds/seed_puzzles.json
    python -m app.modules.puzzles.importer.load seeds/seed_puzzles.json --verify

应用启动时若公共题库为空，会自动导入 backend/seeds/ 下的全部种子文件
（见 app/modules/puzzles/seeding.py）；本脚本用于扩容、更新与自定义题库导入。
"""
...
from app.core.models import Puzzle, SessionLocal, init_db
...
def import_items(
    ...
    mate_check: bool = False,
) -> tuple[int, int, int]:
    """把题目字典列表写入会话（不提交），返回 (新增, 跳过重复, 校验失败)。

    engine 为 UciEngine 实例时逐题校验正解第一手；mate_check 用内置规则
    校验单步杀法题。启动播种与 CLI 共用此逻辑。
    """
    added = skipped = bad = 0
    if mate_check:
...
def load(path: str, verify: bool = False, movetime_ms: int = 1000, mate_check: bool = False) -> None:
    ...
    with SessionLocal() as db:
        added, skipped, bad = import_items(
            db, data, engine=engine, movetime_ms=movetime_ms, mate_check=mate_check
        )
        db.commit()
